...
const lockedKeySet = computed(() => new Set((props.lockedKeys || []).map((k) => String(k))))
...
const canRemoveField = (field) => {
  ...
  return !lockedKeySet.value.has(String(field?.key || ''))
}

const isKeyLocked = (field) => isBuiltin.value && lockedKeySet.value.has(String(field?.key || ''))
  ...
  >
    <template v-if="draft">
      <p v-if="isBuiltin" class="dlg-lead">
        内置模板：<strong>key 不可改</strong>，可编辑显示名与各选项；改动相对出厂定义保存，升级 Nexus 仍保留你的定制。
        也可追加新字段。
      </p>
      <p v-else class="dlg-lead">
        静态侧写可直接改值；动态流转按选项顺序写回（用例/能力/手动须符合转移规则）。
      </p>
      ...
      <section v-if="isBuiltin && baseFields.length" class="base-fields">
        <div class="sec-title">出厂内置 key（不可删改 key）</div>
        <div class="chip-list">
      ...
      <div class="sec-head">
        <div>
          <div class="sec-title">{{ isBuiltin ? '模板字段（可编辑）' : '状态字段' }}</div>
          <div class="sec-sub">侧写：是/否或 1–4；流转：按步骤顺序配置选项（value 建议英文 slug）。</div>
        </div>
      ...
      <el-empty
        ...
        description="暂无字段；内置模板若为空请先重启 Nexus 拉 catalog"
        :image-size="64"
      />
