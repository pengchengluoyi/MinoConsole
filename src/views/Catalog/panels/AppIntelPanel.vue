...
const loadGaps = async () => {
  ...
  try {
    ...
    gapsData.value = null
    ElMessage.error(apiErrorMessage(e, '缺口扫描失败'))
  } finally {
...
const loadProposals = async () => {
  ...
  try {
    ...
    proposals.value = []
    ElMessage.error(apiErrorMessage(e, '读取待审队列失败'))
  } finally {
...
const runAsk = async () => {
  ...
  try {
    ...
    ElMessage.error(apiErrorMessage(e, '问答失败（需配置大模型 Key）'))
  } finally {
    asking.value = false
  }
}
...
<template>
  <div class="intel-panel" :class="{ embedded }">
    <header v-if="!embedded" class="settings-page-header">
      <div>
        <h2 class="settings-page-title">信息基座</h2>
        <p class="settings-page-desc">导航、执行知识、文档库三渠道统一检索与缺口治理</p>
      </div>
    </header>
